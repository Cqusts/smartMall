...
**以数据中台为心脏、以数据飞轮为驱动的电商多模态 AI Agent 体系**

从 AI 客服出发，串起素材生产、直播切片、销售考核与模型微调，形成可自我增强的闭环。

[快速开始](#-快速开始) · [架构设计](#-架构设计) · [文档](#-文档) · [评测结论](evals/README.md)
...
[![Tests](https://img.shields.io/badge/tests-1352%20passing-brightgreen.svg)](#跑测试)

<img src="docs/images/storefront.png" alt="smartMall 店铺前台" width="88%">
...
电商场景里，AI 能力往往是割裂的：客服机器人一套、生成宣传图一套、直播剪辑一套、
质检考核又一套。每套各自攒数据、各自调模型，数据不流动，能力不叠加。

smartMall 的主张是：**把所有 AI 能力接到同一个数据中台上，让它们互相喂养。**

| 谁产出 | 经过什么 | 谁消费 |
|---|---|---|
| 运营 Agent 的宣传图 / 视频 | 素材中心 → 数据中台清洗 | 客服答"这件什么面料"时甩出来的图 |
| 主播的口播话术 | 自动切片 → 转写清洗 | 客服的卖点知识 |
| 客服的真实对话 | Trace 回流 → 四道清洗 | 一份喂 RAG，一份喂微调 |
| 同一批对话 | 指标抽取 | 销售考核 |
...
| 要求 | 版本 | 备注 |
|---|---|---|
| JDK | 21 | 起 5 个 Java 服务 |
| MySQL | 8 | 本机装的就行 |
| Python | 3.11 | 起 AI 侧与店铺前台 |

**只要这三样 —— 不需要 Docker，不需要 GPU。** Kafka / Milvus / ClickHouse
那套属于完整 Docker 部署，本地开发用不到。
...
.\smartmall.ps1 up               # 后台起全部服务，等到就绪才返回
.\smartmall.ps1 serve            # 另开一个终端
```

打开 <http://127.0.0.1:9002/> 就能逛、搜、下单、跟客服对话。

演示账号 `demo` / `buyer2`（买家）、`merchant`（商家），密码统一 `smartmall123`。
> ⚠️ 这是公开的演示口令，别在任何真实环境里用这份种子数据。

### 起不来先自检

```bash
make doctor     # 一次查完 JDK、依赖、数据库、账号、迁移、端口
make status     # 5 个服务各自的状态与 MySQL 连通性
```
...
> **三个 Python 包要分开跑，不能合成一条 `pytest`。** ai-agent 与 ai-rag 的顶层包
> **都叫 `app`**，合在一次收集里先导入的那个会赢，另一个报
> `ModuleNotFoundError: No module named 'app.xxx'`。这是包命名的历史包袱，不是测试坏了。
...
```bash
# 模型从 huggingface.co/Xenova/bge-small-zh-v1.5 下 onnx/model.onnx 与 tokenizer.json
export EMBEDDING_ONNX_DIR=/path/to/bge-small-zh
cd apps/python/ai-rag && python -m app.eval_cli --gate --sweep 0.1,0.5,1.0 --holdout
```

更详细的走查见 [16 · 使用手册](docs/16-walkthrough.md)。
...
- 下单 → 支付 → 发货 → 收货 → 完成
- 取消、超时自动释放库存、退款审核
- 防超卖（条件 `UPDATE`）与幂等（`request_id` 唯一索引）

</td><td width="50%" valign="top">

**鉴权与前后台**
- JWT 签发与校验，身份来自签名令牌**不是请求参数**
- 买家端：列表 / 详情 / 搜索 / 收藏 / 下单 / 客服
- 商家端：上架、看订单、发货、审退款、审素材
- 注册**只能注册出买家**，商家账号只从种子或 DBA 来

</td></tr>
</table>

<div align="center">
<img src="docs/images/product-detail.png" alt="商品详情与登录守卫" width="49%">
<img src="docs/images/merchant-console.png" alt="商家后台" width="49%">
</div>
...
四个共用一套编排底座（`app/agent/nodes.py` 的 `run_node`），页面上的执行轨迹面板
对四个都有效。它们靠一张 `agent_task` 表派活：客服答不上来 → 知识运维补写 →
运营更新文案，中间没有人点按钮。
...
| `intent` | 101 | 七类意图分类 |
| `negative` / `safety` | 20 / 25 | 拒答；注入与违禁拦截，同时不误伤正常提问 |

实测结论写在 [evals/README.md](evals/README.md)，**包括与预期相反的那些** ——
等权 RRF 融合的 Recall@5（0.957）低于纯向量（0.974），机理与留出验证一并记着。
...
<div align="center">
<img src="docs/images/architecture.png" alt="smartMall 总体架构" width="92%">
</div>

> 时序图与服务间调用细节见 [01 · 总体架构](docs/01-architecture.md)。

### 三条设计主张

**1. 数据中台是心脏，不是配角。**
所有 Agent 要么是它的生产者，要么是消费者。整个体系收敛到两张表：
`knowledge_item`（进 RAG）与 `sft_sample`（进微调）。"素材 → RAG"、
"直播切片 → RAG"、"对话 → 微调"是同一套管道的三个入口，不是三套系统。
...
**3. 鉴权放在被访问的那一端，不是只放在网关上。**
Python 侧直接调 `mall-product:8081`，只在网关拦等于没拦。前端藏按钮、
跳登录页都只是体验，判定在 `@RequireMerchant` 与 `AuthService` 上。
...
复制 `deploy/.env.example` 为 `deploy/.env` 后按需修改。
**`.env` 已在 `.gitignore` 里，不要提交。**

<details open>
<summary><b>必需</b></summary>
...
> 管理员密码与应用密码是两个东西。混用会拼出 `smartmall/<root密码>` 这种
> 不存在的组合，而现象是"迁移成功、应用连不上"。

</details>

<details>
<summary><b>模型</b>（用到 AI 能力时才需要）</summary>
...
Docker 全家桶（Kafka / Milvus / ClickHouse / SeaweedFS / Langfuse）另有一批端口与
凭据变量，见 `deploy/.env.example` 与 [deploy/README.md](deploy/README.md)。
...
| 图像 / 视频 | 百炼 `qwen-image-2.0-pro`（同步）· `wan2.7-t2v`（异步轮询） |
| 语音 | FunASR（ASR）· CosyVoice2（TTS） |
| 训练 | LLaMA-Factory（LoRA SFT + DPO）· vLLM |
| 可观测 | Langfuse · Ragas · promptfoo |
| 前端 | 原生 HTML/CSS/JS，零构建、零外部请求 |

**模型策略**：混合部署 —— 对话、文案、图像、视频走云 API，ASR / TTS / Embedding
可本地自建，微调本地 LoRA。

> 图像与视频原方案是本地 ComfyUI + FLUX / Wan2.2，实际改走云 API：单卡 24G 要
> 分时复用，而生成商品图不涉及私有数据、也不需要 LoRA，没有任何必须本地跑的理由。
> 取舍过程见 [02 · 技术选型](docs/02-tech-selection.md)。
...
**总纲与数据**
- [00 · 项目全景](docs/00-overview.md)
- [01 · 总体架构](docs/01-architecture.md)
- [02 · 技术选型](docs/02-tech-selection.md)
- [03 · 数据中台](docs/03-data-platform.md)
- [04 · RAG 知识库](docs/04-rag-knowledge.md)

**Agent 集群**
- [05 · 客服 Agent](docs/05-agent-customer-service.md)
- [06 · 运营 Agent](docs/06-agent-marketing.md)
- [07 · AI 素材中心](docs/07-asset-center.md)
- [08 · 直播切片](docs/08-agent-live-clip.md)
- [11 · Agent 集群协作](docs/11-agent-cluster.md)

</td><td valign="top" width="50%">

**模型与度量**
- [09 · 销售考核](docs/09-sales-kpi.md)
- [10 · 模型微调](docs/10-finetune.md)
- [12 · 评测与可观测](docs/12-eval-observability.md)

**工程落地**
- [13 · 里程碑路线图](docs/13-roadmap.md)
- [14 · 基础设施](docs/14-infra.md)
- [15 · 风险与合规](docs/15-risks-compliance.md)
- [16 · 使用手册](docs/16-walkthrough.md)
...
1. **跑一遍测试**（四条命令见[跑测试](#跑测试)），全绿再提
2. **改了阈值 / 判据要带上量的过程** —— 这个项目里的数字都是测出来的，
   不是拍的（例子见 `evals/README.md` 与各处的代码注释）
3. **注释写"为什么"不写"是什么"** —— 代码本身说得清做了什么，
   说不清的是当初为什么这么选、踩过什么坑
