...
| **运营 Agent** | 离线任务 | 运营发起（异步） | **待审**商品文案 | 商品属性 + 客服埋点 | ⚠️ 文案已实现，图/视频未做 |
...
运营拍脑袋想的卖点是「高级感」，而客服埋点里躺着的事实是：这件衣服被问得
最多的是「会不会起球」，其中两次连客服自己都没答上来。后者才是用户下单前
真正犹豫的地方——把它写进主图，比任何形容词都管用。这一环是数据飞轮在
运营侧的闭合：**对话数据 → 需求分布 → 卖点排序 → 文案**。

链路：`读取商品 → 统计用户关注点 → 收拢依据 → 提炼卖点 → 生成文案 → 广告合规检查 → 写入待审`

为此给 `agent_trace` 补了 `product_id`（迁移 009）。**在这之前这个问题是
答不出来的**：只能拿转人工工单来猜，而工单只记录了答不上来的那些，
是有偏的样本——「会起球吗」一旦答得上来，就永远不会出现在里面。
...
|---|---|---|
| 触达 | 一对一、即时 | 印在详情页、一对多 |
| 说错的后果 | 一次失言 | 可被投诉、可被处罚的广告违法 |
| 责任主体 | — | **店铺**，不是模型 |

所以三类检查一律**只拦截、不改写**：把「最好」自动改成「很好」看着无害，
但改完就没人再看一眼了。

1. **属性冲突** —— 文案说羊绒而属性写羊毛。这是虚假宣传，也是模型最爱犯的错
   （它会顺手把「优质」升级成「羊绒」）。材质词表与 VLM 那条链路**共用同一份**
   （`smartmall_pipeline.vision`），两处各写一份必然渐行渐远
2. **极限词** —— 《广告法》第九条
3. **数字没出处** —— 借知识运维那条数值判据

第 2 条分成两档，是实测逼出来的：一句再正常不过的「很多人买羊毛衫**最担心**
的就是起球」会被 `最X` 模式判成极限词「最担」——而这句话根本没在夸商品。
所以**列出来的那份词表拦截，兜底模式只标记**。文案一律要人工过一遍，标记
就够让人看见；而拦错的代价是这道闸门失去信用，然后被整个绕过去。

#### 没做的部分

docs/06 第 3、4 节的宣传图（ComfyUI + IP-Adapter + CatVTON）和宣传视频
（Wan2.2 + CosyVoice2）**没有实现**。它们需要 24G 显卡与常驻的 ComfyUI，
本机跑不起来，**也没有写成"接口先留着"的空壳**——那种代码看着像做完了，
实际一行都没验证过，比明说没做更糟。
