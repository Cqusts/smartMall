...
| **M2 文本 AI 客服** | 🟡 核心已完成 | LangGraph 状态机、七类意图分流、引用溯源、转人工、Trace 落库、点赞点踩、知识盲点回流、只读业务工具（含越权校验）、WebSocket 流式与 Web 调试台（158 测试）<br/>待做：Redis 会话、意图分类评测集 |
...
单文件前端，零外部依赖（演示环境常常没有外网，「打不开」比「不好看」严重）。
左边是对话，右边是**诊断面板**——意图分到哪一类、命中了什么、相似度多少、
有没有词汇支撑、调了哪些工具、为什么转人工。聊天界面谁都能做，
能看见「为什么这么答」的不多，这块面板才是这个页面存在的理由。
