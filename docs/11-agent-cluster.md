...
| Agent | 类型 | 触发方式 | 主要产出 | 消费的数据 |
|---|---|---|---|---|
| **客服 Agent** | 在线交互 | 用户消息（同步） | 对话回复 + Trace | RAG 知识库 |
| **运营 Agent** | 离线任务 | 运营发起（异步） | 文案 / 图 / 视频 | 商品属性 + 高频问题 |
| **切片 Agent** | 事件驱动 | 直播录制完成 | 切片素材 + 口播话术 | 直播排品表 |
| **数据治理 Agent** | 定时批处理 | Airflow 调度 | 清洗后的知识与样本 | ODS 原始数据 |
| **考核 Agent** | 定时批处理 | 每日凌晨 | 会话评分与归因 | 会话明细 + 订单 |
